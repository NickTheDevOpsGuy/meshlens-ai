...
var getPath = (request) => {
  ...
  for (; i < url.length; i++) {
    ...
    if (charCode === 37) {
      ...
      return tryDecodeURI(
        path2.includes("%25") ? path2.replace(/%25/g, "%2525") : path2
      );
  ...
  #buildMatcher(method) {
    ...
    [this.#middleware, this.#routes].forEach((r) => {
      ...
      if (ownRoute.length !== 0) {
        ...
        routes.push(
          ...Object.keys(r[METHOD_NAME_ALL]).map((path2) => [
            path2,
            r[METHOD_NAME_ALL][path2],
          ])
        );
      }
    });
...
var Node2 = class _Node2 {
  ...
  search(method, path2) {
    ...
    return [
      handlerSets.map(({ handler: handler2, params }) => [handler2, params]),
    ];
  }
};
...
var SAMPLES_DIR = resolveSamplesDir();
var samplesVersion = 0;
...
app.get("/api/health", async (c) =>
  c.json({
    status: "ok",
    tarsConfigured: !!process.env.TETRATE_API_KEY,
  })
);
