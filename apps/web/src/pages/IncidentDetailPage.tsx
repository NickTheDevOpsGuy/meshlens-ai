...
export default function IncidentDetailPage() {
  ...
  const [aiAnalysis, setAiAnalysis] = useState<AIRootCauseAnalysis | undefined>(
    incident?.aiAnalysis
  );
  ...
  const relatedIncidents = useMemo(() => {
    ...
    return incidents
      .filter(
        (i) =>
          i.id !== incident.id &&
          i.affectedServices.some((s) => services.has(s))
      )
      .slice(0, 5);
  }, [incidents, incident]);
  ...
  const traceUrl =
    incident.traceId && settings.traceUrl
  ...
  const handleExport = () => {
    const blob = new Blob(
      [
        JSON.stringify(
          { ...incident, aiAnalysis: aiAnalysis ?? incident.aiAnalysis },
          null,
          2
        ),
      ],
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const addText = (
        text: string,
        opts?: { bold?: boolean; section?: boolean }
      ) => {
      ...
      y += 4;
      if (incident.summary) {
        addText(incident.summary, { section: true });
        y += 2;
      }
      ...
      const res = await fetch("/api/notify", {
        ...
        body: JSON.stringify({
          ...
          pagerdutyIntegrationKey:
            settings.pagerdutyIntegrationKey || undefined,
        }),
      });
      const data = await res.json();
        ...
        {aiAnalysis ? (
          <>
            ...
            <p className="mt-4 text-xs text-slate-500">
              Confidence: {(aiAnalysis.confidence * 100).toFixed(0)}% · Analyzed
              at {new Date(aiAnalysis.analyzedAt).toLocaleString()}
            </p>
          </>
        ) : (
          <p className="text-slate-500 text-sm">
            Click &quot;Analyze with AI&quot; to get root cause analysis.
            Requires TETRATE_API_KEY in .env.
          </p>
        )}
      </section>
      ...
      {relatedIncidents.length > 0 && (
        <section className="mt-8 p-6 rounded-xl border border-slate-800 bg-slate-900/50">
          ...
          <p className="text-sm text-slate-500 mb-3">
            Other incidents affecting the same services:
          </p>
          <ul className="space-y-2">
