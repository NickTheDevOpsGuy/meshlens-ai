...
export default function TimelinePage() {
  ...
  const maxTime = sorted.length
    ? Math.max(...incidents.map((i) => new Date(i.updatedAt).getTime()), minTime + 86400000)
    : Date.now();
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    ...
                    style={{ width: `${Math.max(width * 4, 80)}px`, marginLeft: `${left * 2}%` }}
                  />
                  ...
                  <span className="text-xs text-slate-500 shrink-0">{inc.severity}</span>
                </div>
              </Link>
            );
          })}
      ...
      {sorted.length === 0 && (
        <p className="text-slate-500 text-center py-12">No incidents to display</p>
      )}
    </div>
  );
}
