...
export const sampleIncidents: IncidentBundle[] = [
  {
    ...
    summary:
      "Payment gateway experiencing 503 errors, causing order-service failures. Database connection pool exhaustion suspected.",
    rootCause:
      "PostgreSQL connection pool exhausted due to slow queries and connection leaks in payment-gateway.",
    runbookUrl: "/runbooks/payment-cascade",
    aiAnalysis: {
      summary:
        "Cascade failure originating from payment-gateway's inability to acquire database connections.",
      rootCause:
        "Connection pool exhaustion in payment-gateway. Slow queries (8s+ p99) and a connection leak in the refund handler are holding connections open. Postgres max_connections (100) reached.",
      affectedPath: [
  ...
  },
  {
    ...
    dependencyGraph: {
      ...
      edges: [
        {
          source: "api-gateway",
          target: "recommend-service",
          requestRate: 3400,
          errorRate: 0.15,
          p99LatencyMs: 890,
        },
        {
          source: "recommend-service",
          target: "ml-inference",
          requestRate: 3100,
          errorRate: 0.22,
          p99LatencyMs: 1200,
    ...
    summary:
      "ML inference service returning timeouts. Recommend-service error rate spiked to 15%.",
    runbookUrl: "/runbooks/ml-timeouts",
    traceId: "abc123def456",
    aiAnalysis: {
      summary:
        "Downstream ML inference service is timing out, causing recommend-service to fail.",
      rootCause:
        "ML inference GPU pod scaled to 0 during low-traffic window. Cold start + queue backlog causing 30s+ latencies and timeouts.",
      affectedPath: [
        "api-gateway → recommend-service",
        "recommend-service → ml-inference",
      ],
      recommendations: [
  ...
  },
  {
    ...
    summary:
      "P99 latency for token validation increased from 25ms to 180ms. Resolved after Redis failover.",
    rootCause: "Redis primary failover caused temporary connection churn.",
    aiAnalysis: {
      summary:
        "Redis failover event caused auth-service connection pool to thrash.",
      rootCause:
        "Scheduled Redis maintenance triggered failover. Auth-service Redis client did not handle failover gracefully—connection resets caused retry storms.",
